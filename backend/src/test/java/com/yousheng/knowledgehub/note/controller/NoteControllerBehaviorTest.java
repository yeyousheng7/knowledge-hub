...
class NoteControllerBehaviorTest {
    ...
    void unpublishNote_deletedNote_returns40401() throws Exception {
        ...
        Long noteId = insertNote(user.getId(), "Deleted Public Note", "deleted content", "deleted summary", 1, LocalDateTime.now());
    ...
    void listPublicNotes_returnsOnlyPublicNormalNotDeleted() throws Exception {
        ...
        Long privateNormal = insertNote(user.getId(), "Private Normal", "content", "summary", 0, null);
        ...
        Long publicDeleted = insertNote(user.getId(), "Public Deleted", "content", "summary", 1, LocalDateTime.now());
        jdbcTemplate.update(
                "UPDATE note SET visibility = 'PUBLIC', moderation_status = 'NORMAL', published_at = ? WHERE id = ?",
    ...
    void listPublicNotes_doesNotReturnDeletedNotes() throws Exception {
        ...
        Long noteId = insertNote(user.getId(), "Deleted Public Note", "content", "summary", 1, LocalDateTime.now());
        jdbcTemplate.update(
                "UPDATE note SET visibility = 'PUBLIC', moderation_status = 'NORMAL', published_at = ? WHERE id = ?",
    ...
    void getPublicNoteDetail_privateDeletedTakenDown_returns40401() throws Exception {
        ...
        Long deletedNoteId = insertNote(user.getId(), "Deleted Detail", "content", "summary", 1, LocalDateTime.now());
        jdbcTemplate.update(
                "UPDATE note SET visibility = 'PUBLIC', moderation_status = 'NORMAL', published_at = ? WHERE id = ?",
                LocalDateTime.of(2026, 6, 3, 12, 0),
                deletedNoteId
